...
from style_kb.models import Chunk, ProviderSource, SourceRef, StyleClaim, TimelineEvent
from style_kb.pipeline.base import Stage, StageContext, StageResult
from style_kb.stages.common import load_chunks, load_style_claims, load_timeline_events, read_payload
...
_SCHEMA_VERSION = 2
_CLAIM_TYPES = {"rule", "recommendation", "warning", "definition", "example", "exception"}
...
def _claims_from_payload(payload: Any, chunk: Chunk, context: StageContext) -> list[StyleClaim]:
    ...
    for index, raw_claim in enumerate(raw_claims, start=1):
        if not isinstance(raw_claim, dict):
            validation_errors.append(f"claim {index} is not an object")
            continue
        claim_errors = _raw_claim_validation_errors(raw_claim, context, index=index)
        if claim_errors:
        ...
        try:
            ...
            validation_errors.append(f"claim {index} cannot be materialized: {type(error).__name__}: {error}")
...
def _raw_claim_validation_errors(
    ...
    *,
    ...
    confidence = _clean_string(raw_claim.get("confidence")) or "medium"

    if claim_type not in _CLAIM_TYPES:
        errors.append(f"claim {index} has invalid claim_type={claim_type!r}")
    if confidence not in _CONFIDENCE_LEVELS:
        errors.append(f"claim {index} has invalid confidence={confidence!r}")
    if not subject or not claim_text or not rationale or not evidence:
        errors.append(f"claim {index} contains an empty required field")

    normalized_subject = _normalize_key(subject)
    if normalized_subject in _SERVICE_SUBJECTS:
        errors.append(f"claim {index} has service subject={subject!r}")
...
def _claims_are_valid(
    ...
    if summary.get("chunks_count") != len(chunks):
        return False
    if not isinstance(summary.get("raw_files"), list) or len(summary["raw_files"]) != len(chunks):
        return False
...
def _load_cached_chunk_result(
    ...
    try:
        ...
        return None
    _log_claim_cache(context, chunk=chunk, cache_path=raw_path, cache_hit=True, validation_errors=[])
